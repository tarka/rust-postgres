...
fn test_non_default_database() {
    or_panic!(Connection::connect("postgres://postgres@postgres/postgres", &SslMode::None));
}

#[test]
fn test_url_terminating_slash() {
    or_panic!(Connection::connect("postgres://postgres@postgres/", &SslMode::None));
}

#[test]
fn test_prepare_err() {
    let conn = or_panic!(Connection::connect("postgres://postgres@postgres", &SslMode::None));
...
fn test_unknown_database() {
    match Connection::connect("postgres://postgres@postgres/asdf", &SslMode::None) {
...
fn test_lazy_query_wrong_conn() {
    let conn1 = or_panic!(Connection::connect("postgres://postgres@postgres", &SslMode::None));
    let conn2 = or_panic!(Connection::connect("postgres://postgres@postgres", &SslMode::None));
...
fn test_custom_notice_handler() {
    ...
    let conn = or_panic!(Connection::connect(
            "postgres://postgres@postgres?client_min_messages=NOTICE", &SslMode::None));
...
fn test_cancel_query() {
    ...
    let _t = thread::spawn(move || {
        thread::sleep_ms(500);
        assert!(postgres::cancel_query("postgres://postgres@postgres", &SslMode::None,
                                       cancel_data).is_ok());
    });
...
fn test_require_ssl_conn() {
    ...
    let conn = or_panic!(Connection::connect("postgres://postgres@postgres",
...
fn test_plaintext_pass() {
    or_panic!(Connection::connect("postgres://pass_user:password@postgres/postgres", &SslMode::None));
}

#[test]
fn test_plaintext_pass_no_pass() {
    let ret = Connection::connect("postgres://pass_user@postgres/postgres", &SslMode::None);
    match ret {
...
fn test_plaintext_pass_wrong_pass() {
    let ret = Connection::connect("postgres://pass_user:asdf@postgres/postgres", &SslMode::None);
    match ret {
...
#[test]
fn test_md5_pass() {
    or_panic!(Connection::connect("postgres://md5_user:password@postgres/postgres", &SslMode::None));
}

#[test]
fn test_md5_pass_no_pass() {
    let ret = Connection::connect("postgres://md5_user@postgres/postgres", &SslMode::None);
    match ret {
        Err(ConnectError::MissingPassword) => (),
        Err(err) => panic!("Unexpected error {:?}", err),
        _ => panic!("Expected error")
...
fn test_md5_pass_wrong_pass() {
    let ret = Connection::connect("postgres://md5_user:asdf@postgres/postgres", &SslMode::None);
    match ret {
        Err(ConnectError::DbError(ref e)) if e.code() == &InvalidPassword => {}
        Err(err) => panic!("Unexpected error {:?}", err),
        _ => panic!("Expected error")
...
fn url_encoded_password() {
    let params = "postgresql://username%7b%7c:password%7b%7c@postgres".into_connect_params().unwrap();
